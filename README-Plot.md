...
In opened window, you are able to manage all existent configurations: apply default config to each chart type, remove configuration or add new one:

![Configurations](doc/plot_2.png)     
 
 To add new configuration, you should select json file, which corresponds to the json schema, available in [project sources](https://gitlab.com/shupakabras/kdb-intellij-plugin/raw/master/resources/plot-schema.json).
 
![Select path](doc/plot_3.png)

## Configure Intellij IDEA for json schema validation. 

You could also configure IDEA to validate json file throw the JSON schema.
...
Now you able to add files, which should be validated against the schema. So you could specify single file, or a directory: 

![JSON files set](doc/plot_7.png)

And now, all your files under specified directory would be validated be IDEA. So you could have inline suggestions:
